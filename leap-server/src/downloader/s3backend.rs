...
impl S3Backend {
    pub async fn new(bucket: &str, aws_config: Option<&crate::cfg::AwsConfig>) -> anyhow::Result<Self> {
        ...
        let (access_key, secret_key, region) = if let Some(cfg) = aws_config {
            ...
            (cfg.access_key_id.clone(), cfg.secret_access_key.clone(), cfg.region.clone())
        } else {
            // Fall back to environment variables
            let access_key = std::env::var("AWS_ACCESS_KEY_ID")
                .map_err(|_| anyhow::anyhow!("AWS_ACCESS_KEY_ID not set in environment or config"))?;
            let secret_key = std::env::var("AWS_SECRET_ACCESS_KEY")
                .map_err(|_| anyhow::anyhow!("AWS_SECRET_ACCESS_KEY not set in environment or config"))?;
        ...
        client.head_bucket().bucket(bucket).send().await.map_err(|e| {
    ...
    async fn get_s3_object(&self, key: &str) -> Result<aws_sdk_s3::operation::get_object::GetObjectOutput, Error> {
        ...
        self.client
            ...
            .await
            .map_err(|e| {
                tracing::error!("Failed to get S3 object s3://{}/{}: {}", self.bucket, key, e);
                ...
                Error::IoError(std::io::Error::new(
                    ...
                    format!("Failed to get S3 object s3://{}/{}: {}", self.bucket, key, e)
                ))
            })
    }
}
